...
export default function SingleDay({ match }) {
    ...
    useEffect(() => {
        axios.post('http://localhost/physiosystem/server/client/getSingleBought', { user_id: localStorage.getItem('op'), package_id: match.params.id }).then(res => {
            setSingle(res.data)
        })
    }, [match.params.id])
    if (single) {
        ...
        <div className="singleday flex fd-column ai-start" >
            ...
            <div className="singleday-details flex ai-start">
                <div className="singleday-details-left">
                    <img src={`http://localhost/physiosystem/server/files/${single && single.package.photo}`} className="img-res" alt="" />
                </div>
                ...
                <div className="singleday-videos-items flex ai-center fd-column">
                    {single && day[0].videos.map(video => (
                        <div className="singleday-videos-item">
                            <video controls className="img-res" src={`http://localhost/physiosystem/server/files/${single && video.video_src}`}></video>
                        </div>
                    ))}
                </div>
            </div>
